#' Mutate Education
#'
#' Mutate a fine-grained education variable into the larger categories we use for MPR. The mutated variable will be called 'education'.
#'
...
#'
#' @export
mutate_education <- function(data, var_name = education_og) {

  data <-
    data %>%
    dplyr::mutate(education = case_when({{var_name}} == "Some high school" ~ "Less than high school",
                                        grepl("ome high sch", {{var_name}}) ~ "Less than high school",
                                        {{var_name}} == "Graduated from high school (Diploma/GED or equivalent)" ~ "Graduated from high school",
                                        grepl("raduated from high", {{var_name}}) ~ "Graduated from high school",
