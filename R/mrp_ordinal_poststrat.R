...
#'
#' @export
mrp_ordinal_poststrat <- function(current_model_epred, # posterior prediction for the regression model of interest
    ...
    posterior_tibble_maker <- function(which_ordinal_rating) {
      ...
      single_ordinal_rating <-
        tibble(rating = which_ordinal_rating,
               proportion = colSums(t(current_model_epred[ , , which_ordinal_rating]) * poststrat_tibble$n) / sum(poststrat_tibble$n),
               draw = 1:1000)
    ...
    subgroup_posterior_tibble_maker <- function(data,
      ...
      single_ordinal_rating <-
        ...
        mutate(rating = which_ordinal_rating,
               proportion = colSums(t(current_model_epred[ , data$case, which_ordinal_rating]) * poststrat_tibble[data$case, ]$n) / sum(poststrat_tibble[data$case, ]$n),
               draw = 1:1000,
               outcome = outcome,
