...
#' @export
gen_us_voter <- function(target_variable = "options",
                         partyid_as_ind_other = TRUE,
                         show_levels = TRUE) {

  if(target_variable == "options") {
    print(glue::glue('Your variable weighting options are...'))
    print(glue::glue('From the 2022 Cooperative Election study: "education_collapse", "gender", "gender_bin", "is_man", "race_collapse", "age", "age_alt", "fam_income", "fam_income_collapse", "region", "urbancity", "pew_bornagain", "partyid", "presvote_2020_three", "presvote_2020_four"'))
  }
  ...
  else if(target_variable == "fam_income") {

    fam_income_weights <- weights::wpct(c('Under $20,000', 'Between $20,000 and $49,999', 'Between $50,000 and $79,999', 'Between $80,000 and $99,999', 'Between $100,000 and $149,999', 'Between $150,000 and $199,999', 'At or above $200,000'),
                                        c(.129, .287, .243, .092, .147, .053, .049))

    if(show_levels == TRUE) {
      print(glue::glue("Levels for {target_variable}: {paste(c('Under $20,000', 'Between $20,000 and $49,999', 'Between $50,000 and $79,999', 'Between $80,000 and $99,999', 'Between $100,000 and $149,999', 'Between $150,000 and $199,999', 'At or above $200,000'), collapse = '; ')}"))
    }

    return(fam_income_weights)
