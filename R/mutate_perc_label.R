...
#'
#' Converts numeric point estimate and confidence interval columns into a formatted percentage label. If linebreak is set to TRUE, it adds a line break before the confidence interval. If linebreak is set to FALSE, the entire label is on one line.
#'
...
mutate_perc_label <- function(data,
                              point_est = "mean",
                              lower = "lower",
                              upper = "upper",
                              size = 5.5,
  ...
  if(linebreak == TRUE) {
    data <- data %>%
      ...
      dplyr::mutate(tmp_point_est = !!sym(point_est),
             tmp_lower = !!sym(lower),
             tmp_upper = !!sym(upper),
             perc_label = glue("**{jimbilben::nice_num(tmp_point_est, 0)}%**<br><span style='color:#757575; font-size:{size}pt'>{jimbilben::nice_num(tmp_lower, 1, remove_lead = FALSE)}% - {jimbilben::nice_num(tmp_upper, 1, remove_lead = FALSE)}%</span>")) %>%
    ...
    data <- data %>%
      ...
      dplyr::mutate(tmp_point_est = !!sym(point_est),
             tmp_lower = !!sym(lower),
             tmp_upper = !!sym(upper),
             perc_label = glue("**{jimbilben::nice_num(tmp_point_est, 0)}%** <span style='color:#757575; font-size:{size}pt'>[jimbilben::{nice_num(tmp_lower, 1, remove_lead = FALSE)}% - {jimbilben::nice_num(tmp_upper, 1, remove_lead = FALSE)}%]</span>")) %>%
      dplyr::select(-tmp_point_est, -tmp_lower, -tmp_upper)
  }
