...
#'
#' @param data The data from which the x variable is to be taken
#' @param variable A character string naming the x variable
...
nice_x <- function(data,
  ...
  if(number_block == FALSE) {
    ...
    scale_x_continuous(limits = c(lower_limit, upper_limit * number_block_multiple),
                       breaks = seq(lower_limit, upper_limit, break_width),
                       expand = expansion(add = c(expand_low, expand_high)))
