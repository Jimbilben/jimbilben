...
#'
#' @export
mrp_psup_poststrat <- function(current_model_epred, # posterior prediction for the regression model of interest
  ...
  if(subgroups == FALSE) {
    ...
    psup_posterior <-
      tibble(superior = colSums(t(current_model_epred[ , , superior_name]) * poststrat_tibble$n) / sum(poststrat_tibble$n), # the t transforms the current_model matrix so it can be multiplied with our poststrat matrix
             inferior = colSums(t(current_model_epred[ , , inferior_name]) * poststrat_tibble$n) / sum(poststrat_tibble$n),
             equal = colSums(t(current_model_epred[ , , equal_name]) * poststrat_tibble$n) / sum(poststrat_tibble$n),
             draw = 1:1000) %>%
    ...
    subgroup_posterior_tibble_maker <- function(data,
      ...
      psup <-
        sample_n(psup,
                 1000,
                 replace = TRUE) %>%
        mutate(superior = colSums(t(current_model_epred[ , data$case, superior_name]) * poststrat_tibble[data$case, ]$n) / sum(poststrat_tibble[data$case, ]$n), # the t transforms the current_model matrix so it can be multiplied with our poststrat matrix
               inferior = colSums(t(current_model_epred[ , data$case, inferior_name]) * poststrat_tibble[data$case, ]$n) / sum(poststrat_tibble[data$case, ]$n),
               equal = colSums(t(current_model_epred[ , data$case, equal_name]) * poststrat_tibble[data$case, ]$n) / sum(poststrat_tibble[data$case, ]$n),
               draw = 1:1000) %>%
        pivot_longer(cols = superior:equal, names_to = "superiority", values_to = "proportion") %>%
        mutate(psup = case_when(superiority == "inferior" ~ 0, # inferior scores 0
                                superiority == "equal" ~ .5 * proportion, # same scores half
