...
#'
#' @export
mrp_categorical_poststrat <- function(current_model_epred, # posterior prediction for the regression model of interest
                                      subgroups = FALSE, # just indicating this is a population-level prediction (FALSE) or a subgrouped version (TRUE)
                                      outcome = "Party ID", # give a name to the outcome/response being assessed, e.g., Support for Vaccines
                                      interval = .95, # the summary interval level
                                      poststrat_tibble = acs5_2020_poststrat, # this is the tibble - usually an ACS tibble - containing the demographic variable names. You can group it!
                                      .point_est = "mean",
                                      .decimals = 1,
                                      .remove_lead = FALSE) {
  if(subgroups == FALSE) {
    ...
    posterior_tibble_maker <- function(which_categorical_choice,
      ...
      single_categorical_choice <-
        tibble(choice = name_categorical_choice,
               proportion = colSums(t(current_model_epred[ , , which_categorical_choice]) * poststrat_tibble$n) / sum(poststrat_tibble$n),
               draw = 1:1000)
    ...
    subgroup_posterior_tibble_maker <- function(data,
      ...
      single_categorical_choice <-
        ...
        mutate(choice = name_categorical_choice,
               proportion = colSums(t(current_model_epred[ , data$case, which_categorical_choice]) * poststrat_tibble[data$case, ]$n) / sum(poststrat_tibble[data$case, ]$n),
               draw = 1:1000,
               outcome = outcome,
